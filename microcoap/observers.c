...
int addCoAPObserver(const char* hostName, long unsigned int* port, coap_packet_t pkt, const coap_endpoint_path_t *path)
{
	if (observersCount < MAX_OBSERVERS_COUNT)
	{
		...
		observers[observersCount] = (coap_observer_t){ hostName, *port, pkt, *path };
		observersCount += 1;
...
bool removeCoApObserver(const char* hostName, long unsigned int* port, coap_endpoint_path_t *path) {
	int i;
